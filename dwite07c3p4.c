/*
DWITE '07 R3 #4 - The Convex Fence Company
Author: Dan Shan
Date: 2025-02-16
Convex Hull: Graham scan
round each edge length up to nearest integer
*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
int ang(struct p a, struct p b, struct p c){ // turn direction
...
int main() {
    for(int ti=0;ti<5;ti++){
        ...
        double c=0;
        for(int i=1;i<r2;i++) c+=ceil(sqrt(dist(stk[i],stk[i-1])));
        c+=ceil(sqrt(dist(stk[0],stk[r2-1])));
        printf("$%0.2lf\n",c*r);
    }
}
